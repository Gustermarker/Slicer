...
public class GeneratePath {
    ...
    public List<Point2d> GeneratePathVertical(List<Point2d> perim_intersections, List<Point3d> perimeter) {
        List<Point2d> temp = new ArrayList<Point2d>();
        List<Point2d> list = new ArrayList<Point2d>();
        System.out.println(perimeter.get(0).getX());
        GeneratePerimeters(perimeter);
        ...
        double X_curr;
        double Y_left;
        double prev_Y_left, prev_Y_right;
        boolean right = true; // is direction of path left or right
        boolean reset = false; // if this condition is true, we restart the while loop from the top
        int index;

        while (list.size() > 0) {
            X_curr = list.get(0).getX();
            Y_left = list.get(0).getY();
            prev_Y_left = list.get(0).getY();
            prev_Y_right = list.get(1).getY();
            reset = true;
            index = 0;
            ...
            do {
                // move to next layer
                for (int j = index; j < list.size(); j += 2) {
                    if (list.get(j).getX() > X_curr) {
                        index = j;
                        X_curr = list.get(j).getX();
                        break;
                    }
                }
                reset = true; // default true, will change if reset condition met
                ...
                //for (int i = index; list.get(i).getY() == Y_curr || i < list.size() - 1; i += 2) {
                for (int i = index; i < list.size() - 1; i += 2) {
                    if (list.get(i).getX() > X_curr) {
                        break;
                    }

                    if (list.get(i).getY() >= prev_Y_left && list.get(i).getY() <= prev_Y_right ||
                            list.get(i).getY() <= prev_Y_left && list.get(i).getY() <= prev_Y_right) {
                        if (right) {
                            path_list.add(list.get(i));
                            path_list.add(list.get(i + 1));
                            prev_Y_left = list.get(i).getY();
                            prev_Y_right = list.get(i + 1).getY();
                            right = false;
                        } else { // left
                            path_list.add(list.get(i + 1));
                            path_list.add(list.get(i));
     ...
     * the weird +/- 0.001 is because floating point rounding error on doubles. ie sometimes when x = y = 1.5,
     * x == y will return false (x = 1.50000001, y = 1.50000000). should have used BigDecimal
     */
    public List<Point2d> GeneratePerimeters(List<Point3d> input) {
